...
public sealed class RecordClaudePromptHookCommandTests
{
    ...
    static RecordClaudePromptHookCommandTests()
    {
    ...
    public async Task Execute_NewSession_WritesMapAndMemoryEvents()
    {
        ...
        var result = await CreateCommand(eventStore, FirstPromptId)
            .Execute(Executor);
        ...
        Assert.IsType<ClaudePromptHookRecordedV1>(
            Assert.Single(
    ...
    public async Task Execute_ExistingSession_ReusesMappedAggregate()
    {
        ...
        var firstMemoryAggregateId =
            firstMapState.AggregateIdsBySession[ThreadId];
        ...
        var recordedEvent = Assert.IsType<ClaudePromptHookRecordedV1>(
            Assert.Single(writtenAggregate.Value.LastExecutedPayloads).EventData
        );
        Assert.Equal(SecondPromptId, recordedEvent.PromptId);
    }

    [Fact]
    public async Task Execute_SameTurn_AppendsPromptAndStopHooks()
    {
        DatabaseFriendlyGuidGenerator.SetDefaultGuidGenerationDatabase(
            Database.SqlServer
        );
        ...
        await CreateCommand(eventStore, FirstPromptId, "UserPromptSubmit")
            .Execute(Executor);
        await CreateCommand(eventStore, FirstPromptId, "Stop")
            .Execute(Executor);

        var memoryState = Assert.IsType<MemoryStateData>(
            Assert.Single(eventStore.LastWritten).Value.StateData
        );
    ...
    public void PersistenceRoundTrip_PreservesJsonElementPayload()
    {
        ...
        var eventRoundTrip = Assert.IsType<ClaudePromptHookRecordedV1>(
            storedEvent.Deserialize().EventData
        );
        var storedMessage = SerializedPayloadMessage.FromPayload(payload);
        var messageRoundTrip = Assert.IsType<ClaudePromptHookRecordedV1>(
    ...
    private static RecordClaudePromptHookCommand CreateCommand(
        CapturingEventStoreWithOutbox eventStore,
        PromptId promptId,
        string hookEventName = "UserPromptSubmit"
    ) =>
        new(CreateHandler(eventStore))
        {
            ThreadId = ThreadId,
            PromptId = promptId,
