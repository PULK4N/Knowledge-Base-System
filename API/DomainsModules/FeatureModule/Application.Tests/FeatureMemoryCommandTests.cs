...
public sealed class FeatureMemoryCommandTests
{
    [Theory]
    ...
    )
    {
        ...
        var command = new AddFeatureCommand(handler, new StubFeatureSummaryRepository())
        {
            ProjectId = Guid.NewGuid(),
            Name = "feature-name",
            Summary = "Summary",
            Status = "Starting",
            SessionId = FeatureStateDataProvider.SessionId
        };
        if (suppliedMemoryId)
            command.MemoryAggregateId = FeatureStateDataProvider.MemoryAggregateId.Value;
        if (userOrigin)
        ...
        var payload = Assert.Single(
            outbox.Written,
            eventPayload => eventPayload.EventData is FeatureAddedV2
        );
        ...
        var expectedMemoryId = userOrigin
            ...
            : FeatureStateDataProvider.MemoryAggregateId;
        Assert.Equal(expectedMemoryId, created.MemoryAggregateId);
        var state = (FeatureStateData)created.Apply(
            new FeatureStateData(payload.EventExecutionInfo.AggregateId),
        ...
        Assert.Equal(nameof(FeatureAddedV2), history.EventName);
        ...
        Assert.Equal(nameof(FeatureAddedV2), relation.Relation);
        Assert.Equal(
            AggregateId.FromDatabaseGuid(result.FeatureId),
    ...
    )
    {
        ...
        var handler = new StateMachineHandler(
            new StateCalculator(
                new OrderNumberHelper(), new FeatureStateDataProvider(),
        ...
        var command = new AddFeatureCommand(handler, new StubFeatureSummaryRepository())
        {
            ProjectId = Guid.NewGuid(), Name = "feature", Summary = "summary", Status = "Starting",
    ...
    private static EventPayload CreateSessionMapPayload(AggregateId aggregateId)
    {
        var payload = EventPayload.Create(
            ...
            new SessionAggregateMapAddedV1(
                new ThreadId(FeatureStateDataProvider.SessionId),
                FeatureStateDataProvider.MemoryAggregateId
            )
        );
        ...
        public Task<object> GetStateDataByStateMachine(
            ...
            Task.FromResult<object>(
                stateMachineId switch
                {
                    ...
                    _ => new FeatureStateData(aggregateId)
                }
            );
